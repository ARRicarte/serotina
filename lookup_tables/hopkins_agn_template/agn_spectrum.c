...
/* 
	Script to return the model intrinsic (un-reddened/un-obscured) quasar spectrum
		as a function of frequency and bolometric quasar luminosity, constructed from
		observations from ~300 microns to ~100 keV in Hopkins, Richards, 
		and Hernquist 2006, ApJ (HRH06)

	  These model spectra are compiled in the paper above, which details the 
	    observational data sets, but in short, we draw heavily from the 
	...
	number of input parameters : 1-2
		1 : log_{10}(L_bol/L_sun) -- the log of the bolometric quasar luminosity 
				for which the mean spectrum is desired. This uses the model 
				constructed from the observations above in HRH06, in which the mean
				intrinsic quasar SED is a function only of bolometric luminosity. 
				units are :
					L_sun = the bolometric solar luminosity = 3.9 x 10^33 erg/s
		2 : SDSS_KEY -- (OPTIONAL) keyword. By default, the template spectrum of 
				vanden Berk et al. 2001 from SDSS is overlaid on the optical portion 
				of the spectrum, as described in HRH06. If for some reason you 
				want to turn this feature *off* (for example, you need to smooth 
				over or otherwise don't want to include line details), then 
				add a second input parameter, set to 0

	results are simply printed to stdout, in three columns. The header 
	  printed out will describe them, in the general format : 
	  
			;;//         
			;;// Intrinsic (un-reddened) AGN spectrum for 
			;;//   log_{10}(L_bol/L_sun)   = xx.xx
			;;//   log_{10}(L_bol/[erg/s]) = xx.xx 
			;;//     (throughout, L_sun is the bolometric solar L = 3.9x10^33 erg/s)  
			;;// columns are frequency, AGN spectrum (nu*L_nu) in solar and CGS units :  
			;;//   log_{10}(nu/Hz)   log_{10}(nu*L_nu/L_sun)  log_{10}(nu*L_nu/[erg/s])  
			;;// the output frequency list spans ~300 microns to ~100 keV  
			;;// the last four columns compute bands frequently adopted :  
			;;//   nu = -1.00  ::  B-band (4400 Angstroms)  
			;;//   nu = -2.00  ::  mid-IR (15 microns)  
			;;//   nu = -3.00  ::  soft X-ray (integrated luminosity from 0.5-2 keV)  
			;;//   nu = -4.00  ::  hard X-ray (integrated luminosity from 2-10  keV)  
			;;//         


	Example use : 
        compile       ::    > gcc agn_spectrum.c -o agn_spec

        standard call ::    > agn_spec 14.0
             (print the template spectrum for a L_bol = 10^14 L_sun quasar to stdout)

        dump to file  ::    > agn_spec 14.0 > filename
             (the output results above get dumped to 'filename')

        turn off the SDSS template overlay  ::    > agn_spec 14.0 0
             (as the above, but adds the second input set to 0, to turn off the 
                overlay of the SDSS model spectrum in the optical)
...
{
	double nu, log_L_bol, l_band, *l_band_vec;
	char *MODEL_SPECTRUM_SET;
	char *band_check;
	char *SLOAN_SET;
	int BAND_KEY=0;	int MODEL_SPECTRUM_KEY; int SLOAN_KEY=0; int i_nu;
		// sets if simply calling the bolometric correction for a prefab band : 
		//		BB = B-band, IR = 15 micron, SX = 0.5-2 keV, HX = 2-10 keV	
  	if (argc < 2) {
  	  fprintf(stderr, "Expected >=1 arguments (found %d)\n",argc-1); 
  	  exit(0); 
  	}

int N_nu = 375;	
double nu_vec[375] = {
12.00, 12.02, 12.04, 12.06, 12.08, 12.10, 12.12, 12.14, 12.16, 12.18, 12.20, 12.22, 12.24,
12.26, 12.28, 12.30, 12.32, 12.34, 12.36, 12.38, 12.40, 12.42, 12.44, 12.46, 12.48, 
...
17.02, 17.04, 17.06, 17.08, 17.10, 17.12, 17.14, 17.16, 17.18, 17.20, 17.22, 17.24, 17.26,
17.28, 17.30, 17.32, 17.34, 17.36, 17.38, 17.40, 17.42, 17.44, 17.46, 17.48, 17.50, 17.52,
17.54, 17.56, 17.58, 17.60, 17.62, 17.64, 17.66, 17.68, 17.70, 17.72, 17.74, 17.76, 17.78,
17.80, 17.82, 17.84, 17.86, 17.88, 17.90, 17.92, 17.94, 17.96, 17.98, 18.00, 18.02, 18.04,
18.06, 18.08, 18.10, 18.12, 18.14, 18.16, 18.18, 18.20, 18.22, 18.24, 18.26, 18.28, 18.30,
18.32, 18.34, 18.36, 18.38, 18.40, 18.42, 18.44, 18.46, 18.48, 18.50, 18.52, 18.54, 18.56,
18.58, 18.60, 18.62, 18.64, 18.66, 18.68, 18.70, 18.72, 18.74, 18.76, 18.78, 18.80, 18.82,
18.84, 18.86, 18.88, 18.90, 18.92, 18.94, 18.96, 18.98, 19.00, 19.02, 19.04, 19.06, 19.08,
19.10, 19.12, 19.14, 19.16, 19.18, 19.20, 19.22, 19.24, 19.26, 19.28, 19.30, 19.32, 19.34,
19.36, 19.38, 19.40, -1.00, -2.00, -3.00, -4.00 };
l_band_vec = calloc(N_nu,sizeof(double));

log_L_bol = atof(argv[1]);
SLOAN_KEY = 1;
if (argc >= 3) {SLOAN_KEY  = atoi(argv[2]);}	
   if ((SLOAN_KEY != 0)&&(SLOAN_KEY != 1)) {SLOAN_KEY=0;}
  // if specified, can turn on/off the superposition of the SDSS mean spectrum (default = ON)


	MODEL_SPECTRUM_KEY = 0;  // switch to 1 for the Richards et al. 2006 luminosity-independent spectrum
	for (i_nu=0;i_nu<N_nu;i_nu++) {
	nu = pow(10.,nu_vec[i_nu]);
	if (nu_vec[i_nu]<=0.) {nu=nu_vec[i_nu];}
	if (MODEL_SPECTRUM_KEY==0) l_band = log10(l_band_HRH(log_L_bol,nu));
	if (MODEL_SPECTRUM_KEY==1) l_band = l_band_GTR(log_L_bol,nu);
	double log_nu;
		if (nu <= 0.) {
	...
	// now print the resulting spectrum
	double to_cgs = log10(3.9) + 33.0;
 	 printf(";;//         \n");
 	 printf(";;// Intrinsic (un-reddened) AGN spectrum for \n");
 	 printf(";;//   log_{10}(L_bol/L_sun)   = %5.2f \n",log_L_bol);
 	 printf(";;//   log_{10}(L_bol/[erg/s]) = %5.2f \n",log_L_bol+to_cgs);
 	 printf(";;//     (throughout, L_sun is the bolometric solar L = 3.9x10^33 erg/s)  \n",log_L_bol);
 	 printf(";;// columns are frequency, AGN spectrum (nu*L_nu) in solar and CGS units :  \n");
 	 ...
 	 printf(";;//         \n");	
    for (i_nu=0;i_nu<N_nu;i_nu++) printf(" %8.2f %10.5f %10.5f \n",nu_vec[i_nu],l_band_vec[i_nu],l_band_vec[i_nu]+to_cgs);
